...
df = pd.read_csv("electric_vehicle_spec_2026.csv")
...
plt.ylabel("Brand")
plt.tight_layout()
plt.savefig("Top 10 EV Brands by Number of Models.png")
plt.show()
...
plt.tight_layout()
plt.savefig("Efficiency Distribution (Wh/km).png")
plt.show()
...
plt.tight_layout()
plt.savefig("0–100 km/h Acceleration Time by Brand.png")
plt.show()

# 3. Efficiency Distribution (Histogram)
plt.figure()
sns.histplot(df['efficiency_wh_per_km'].dropna(), bins=20, kde=True, color="teal")
plt.title("Efficiency Distribution (Wh/km)")
plt.xlabel("Efficiency (Wh/km)")
